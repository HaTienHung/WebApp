...
<body>
  <div>
    <main>
      <header class="border-b bg-[#fffefc]">
        <div class="bg-white-200 flex items-center justify-between main-color py-[18px] pl-[180px] pr-[140px]">
          <div>
            <a href="/">
  ...
  <section class="text-black">
    <section class="pl-[140px] relative min-h-screen">
      <div>
      ...
      <div class="flex h-[1000px]">
        <div class="w-[800px] p-10">
          <div>
            ...
            <input type="email" class="p-2 border w-[570px] rounded-md my-2"><br>
            <label>Tên</label><br>
            <input type="email" class="p-2 border w-[570px] rounded-md my-2"><br>
          ...
          <div class="bg-[#F3D9A8] border duration-250 ease-in-out flex h-[48px] hover:scale-[1.009] justify-center rounded-xl text-black transition mt-2">
            <button><a href="{{route('checkout.index')}}" class="bg-[#F3D9A8]">Xác nhận</a></button>
          </div>
        </div>
        <div class="border-l bg-[#F5F5F5]">
          <div class="p-10">
            ...
            <div class="flex py-2">
              ...
              <div class="flex w-[330px] flex-wrap ml-3 text-[14px]">
                <p>{{ $item['name'] }}</p>
              ...
              <div class="flex items-center justify-center">
                <h1 class="text-[12px] min-w-[150px] text-right">{{number_format(floatval(str_replace(['.', ','], ['', '.'], $item['price']))*intval(str_replace(['.', ','], ['', '.'], $item['quantity'])), 2, ',', '.')}} VND</h1>
              </div>
            </div>
            @endforeach
            <div class="justify-between flex w-[525px] py-3">
              <p>Tổng tiền: </p>
              <p>{{number_format( $total, 2, ',', '.')}} VND</p>
            </div>
          </div>
        </div>
      </div>
